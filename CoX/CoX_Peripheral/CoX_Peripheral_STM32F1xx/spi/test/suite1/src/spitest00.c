...
// SPI Group Array
//
unsigned long ulSPI[3] = {SPI1_BASE, SPI2_BASE, SPI3_BASE};
...
// SPI Int type
//
unsigned long ulSpiIntType[3] = {SPI_INT_RX, SPI_INT_TX, 
                                 SPI_INT_ERRE};

//
// SPI DMA type
//
unsigned long ulSpiDMAType[3] = {SPI_DMA_TX, SPI_DMA_RX, SPI_DMA_BOTH};

//
// SPI polynomial set value
//
unsigned long ulSpipolynomial[3] = {1, 7, 10000};
...
//*****************************************************************************
//
//! \brief xspi 001 test of Interrupt enable and disable test.
//!
//! \return None.
//
//*****************************************************************************
static void spiIntConfigure_Test()
{
	   unsigned long i, j;
    ...
    for(i = 0; i < 2; i++)
	  {
			  for(j = 0; j < 3; j++)
			  {
					  SPIIntEnable(ulSPI[i], ulSpiIntType[j]);
					  ulTemp = xHWREG(ulSPI[i] + SPI_CR2) & ulSpiIntType[j];
						TestAssert(( ulSpiIntType[j] == ulTemp), 
												 "xspi Int enable test!");	
        }					  
    }
    ...
    for(i = 0; i < 2; i++)
	  {
			  for(j = 0; j < 3; j++)
			  {
					  SPIIntDisable(ulSPI[i], ulSpiIntType[j]);
					  ulTemp = xHWREG(ulSPI[i] + SPI_CR2) & ulSpiIntType[j];
						TestAssert(0 == ulTemp, 
												 "xspi Int enable test!");	
        }					  
...
//*****************************************************************************
//
//! \brief xspi 001 test of DMA configure.
//!
//! \return None.
//
//*****************************************************************************
static  void spiDMAConfigure_Test()
{
	   unsigned long i, j;
	   unsigned long ulTemp;
    ...
    for(i = 0; i < 2; i++)
	  {
			  for(j = 0; j < 3; j++)
			  {
					  xSPIDMAEnable(ulSPI[i], ulSpiDMAType[j]);
					  ulTemp = xHWREG(ulSPI[i] + SPI_CR2) & ulSpiDMAType[j];
						TestAssert(( ulSpiDMAType[j] == ulTemp), 
												 "xspi DMA enable test!");	
        }					  
    }
		...
		for(i = 0; i < 2; i++)
	  {
			  for(j = 0; j < 3; j++)
			  {
					  xSPIDMADisable(ulSPI[i], ulSpiDMAType[j]);
					  ulTemp = xHWREG(ulSPI[i] + SPI_CR2) & ulSpiDMAType[j];
						TestAssert((0 == ulTemp), 
												 "xspi DMA enable test!");	
        }					  
...
static void spiCRCConfigure_Test()
{
	   unsigned long i, j;
	   unsigned long ulTemp;
	
	   //
	   // CRC polynomial set test
	   //
		for(i = 0; i < 2; i++)
	  {
			  for(j = 0; j < 3; j++)
			  {
					  SPICRCPolSet(ulSPI[i], ulSpipolynomial[j]);
					  ulTemp = xHWREG(ulSPI[i] + SPI_CRCPR) & 0xffff;
						TestAssert((ulSpipolynomial[j] == ulTemp), 
												 "SPICRCPolSet fail!");	
        }					  
    }
	  ...
	  {
		    SPICRCEnble(ulSPI[i]);		
			  ulTemp = xHWREG(ulSPI[i] + SPI_CR1) & SPI_CR1_CRCEN;
        TestAssert(( SPI_CR1_CRCEN == ulTemp), 
		                 "SPICRCEnble error!");		
    }		
...
static void xSpi001Execute(void)
{
    ...
    xspi_SpiLsbMsbSet_test();
	  ...
	  spiIntConfigure_Test();
	  spiDMAConfigure_Test();
	  spiCRCConfigure_Test();
}
