...
// Count detecte phase
//
static unsigned long ulCountPhase[2] = {xTIMER_COUNTER_RISING, xTIMER_COUNTER_FALLING};
...
static void xTimer001Execute(void)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        //        
        xTimerIntEnable(ulBase, i,  TIMER_INT_MATCH);
        ...
        //
        TimerIntDisable(ulBase, TIMER_INT_MATCH);
        ulTemp =  xHWREG(ulBase + TIMER_O_TCSR) & TIMER_INT_MATCH;
