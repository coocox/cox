...
unsigned long ulBodVoltage[] = {SYSCTL_BOD_2_7, SYSCTL_BOD_3_8};
...
static void xsysctl_Bod_test(void)
{
    ...
    TestAssert((0 == (ulTemp & GCR_BODCR_BOD_VL_DIS)), "xsysctl API error!");
 
    SysCtlBODEnable(xfalse);
    ulTemp = xHWREG(GCR_BODCR);
    TestAssert((GCR_BODCR_BOD_VL_DIS == (ulTemp & GCR_BODCR_BOD_VL_DIS)), "xsysctl API error!");
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        ulTemp = xHWREG(GCR_BODCR);
        TestAssert(((i+1) == (ulTemp & GCR_BODCR_BOD_VL_M)>>GCR_BODCR_BOD_VL_S),
                                                          "xsysctl API error!");
    }
