...
void xuart001Execute_NormalConfig(void)
{
    ...
    ulTemp1 = xHWREG(UART_BASE + UART_LCR) & UART_LCR_BCB;
    TestAssert(ulTemp1 == UART_LCR_BCB, "xuart API error!");
...
unsigned long uart0CallbackFunc(void *pvCBData, 
                                       ...
                                       void *pvMsgData)
{
    if((UARTIntStatusGet(UART0_BASE, xtrue) & 0x100) == 0x100)
    {
...
void xuart001Execute_uartInt(void)
{
    ...
    for(i=0; i<dataNum; i++)
    {
        TestAssert(recData[i] == ('0' + i), "xuart API error!");      
    }
}

//*****************************************************************************
//
//! \brief xuart001 test execute main body.
//
//! \return None.
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Get the Test description of xuart002 test.
//!
...
void xuart002Execute_FIFOConfig(void)
{
    ...
    //
    xHWREG(UART_BASE + UART_FCR) = UART_FCR_RX_RST | UART_FCR_TX_RST;

    SysCtlDelay(100000);

    TestAssert(UARTFIFOTxIsEmpty(UART_BASE) == xtrue, "xuart API error!");

    TestAssert(UARTFIFOTxIsFull(UART_BASE) == xfalse, "xuart API error!");

    TestAssert(UARTFIFOTxLength(UART_BASE) == 0, "xuart API error!");

    TestAssert(UARTFIFORxIsFull(UART_BASE) == xfalse, "xuart API error!");
    
    TestAssert(UARTFIFORxIsEmpty(UART_BASE) == xtrue, "xuart API error!");
    
    TestAssert(UARTFIFORxLength(UART_BASE) == 0, "xuart API error!");
...
void xuart003Execute_IrDAConfig(void)
{
    ...
    TestAssert(ulTemp1 == 0x02, "xuart API error!");
    ...
    TestAssert(ulTemp1 == 0x62, "xuart API error!");

    ulTemp1 = ~(UART_CONFIG_IrDA_TX_EN | UART_CONFIG_IrDA_INV_TX | 
                UART_CONFIG_IrDA_INV_RX);
    UARTIrDAConfig(UART_BASE, ulTemp1);
    ulTemp1 = xHWREG(UART_BASE + UART_IRCR) & 0x62;
    TestAssert(ulTemp1 == 0x0, "xuart API error!");
...
void xuart004Execute_ModemConfig(void)
{
    ...
    for(i=0; i<modemStNum; i++)
    {
        ...
        TestAssert(ulTemp1 == ulModemSt[i], "xuart API error!");
    }
}
//*****************************************************************************
//
...
// xuart Config test case struct.
//
const tTestCase sTestxuart004Modem = {
    xuart004GetTest,
    xuart004Setup,
    xuart004TearDown,
    xuart004Execute
...
void xuart005Execute_flowControlConfig(void)
{
    ...
    for(i=0; i<flowCtlNum; i++)
    {
        ...
        TestAssert(ulTemp1 == ulFlowCtlCfg[i], "error!");
    }  
}
//*****************************************************************************
//
