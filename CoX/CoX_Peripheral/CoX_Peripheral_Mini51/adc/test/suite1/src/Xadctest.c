...
static void adcapi_config( void )
{
    ...
    Test Sample_Test[] = { { ADC_BASE, ADC_INPUT_SINGLE, 0,
    ...
    for ( ; i < ullength; i++ )
    {
        ...
        ultemp = xHWREG( Sample_Test[i].ulBase + ADC_CR ) & (ADC_CR_TRGEN | ADC_CR_TRGCOND_FALLING);
...
static void adcapi_cmpdata( void )
{
    ...
    for ( ; i < ullength; i++ )
    {
        ADCConfigure( ADC_BASE, ADC_OP_SINGLE_CYCLE, 0 , ADC_TRIGGER_PROCESSOR );
...
static void adcapi_adc( void )
{
    ...
    ulTemp = xHWREG( ADC_BASE + ADC_CR ) & ADC_CR_ADST; 
    TestAssert( 0x800 == ulTemp, "xadc API \"ADCProcessorTrigger()\" error!" );
...
static void xadc_config( void )
{
    ...
    for (; i < ullength; i++)
    {
        ...
        ultemp = xHWREG( Sample_Test[i].ulBase + ADC_CR ) & 
                            ( ADC_CR_TRGEN | ADC_CR_TRGCOND_FALLING );
        TestAssert( ultemp == Sample_Test[i].ulResult, 
...
static void xadc_stepconfig( void )
{
    ...
    Test Sample_Test1[] = { { xADC0_BASE, 0, xADC_CTL_CH0, 0x01, 0 },
                            { xADC0_BASE, 1, xADC_CTL_CH1, 0x03, 0 },
                            { xADC0_BASE, 2, xADC_CTL_CH2, 0x07, 0 },
                            { xADC0_BASE, 3, xADC_CTL_CH3, 0x0f, 0 }
                          };
    
    Test Sample_Test2[] = { { xADC0_BASE, 0, xADC_CTL_CH0 | xADC_CTL_CMP0, 0x01, 0x00 },
                            { xADC0_BASE, 1, xADC_CTL_CH1 | xADC_CTL_CMP1, 0x03, 0x08 }
                          };
    ...
    for ( ; i < ullength1; i++ )
    {
        xADCStepConfigure( Sample_Test1[i].ulBase, Sample_Test1[i].ulStep, Sample_Test1[i].ulConfig );
        ultemp[0] = xHWREG( Sample_Test1[i].ulBase + ADC_CHER ) & ( ADC_CHER_CHEN_M );
        if ( xADC_CTL_CMP0 & Sample_Test1[i].ulConfig )
        {
            ultemp[1] = xHWREG( Sample_Test1[i].ulBase + ADC_CMPR0 ) & ADC_CMPRX_CMPCH_M;
        } else if( xADC_CTL_CMP1 & Sample_Test1[i].ulConfig )
        {
            ultemp[1] = xHWREG( Sample_Test1[i].ulBase + ADC_CMPR1 ) & ADC_CMPRX_CMPCH_M;
        }
        else
        {
            ultemp[1] = 0;
    ...
    for ( i = 0; i < ullength2; i++ )
    {
        ...
        ultemp[0] = xHWREG( Sample_Test2[i].ulBase + ADC_CHER ) & ( ADC_CHER_CHEN_M );
        if ( xADC_CTL_CMP0 & Sample_Test2[i].ulConfig )
        {
            ultemp[1] = xHWREG( Sample_Test2[i].ulBase + ADC_CMPR0 ) & ADC_CMPRX_CMPCH_M;
        } else
        {
            ultemp[1] = xHWREG( Sample_Test2[i].ulBase + ADC_CMPR1 ) & ADC_CMPRX_CMPCH_M;
        }
...
static void xadc_compregset( void )
{
    ...
    Test Sample_Test[] = { { xADC0_BASE, 0,  0x0FF, 0x0FF, 0x0FF0100},
...
static void xadc002Setup( void )
{
    ...
    // Configure ADC
    //
    ADCConfigure( ADC_BASE, ADC_INPUT_SINGLE, 0, ADC_TRIGGER_PROCESSOR );
    //
