...
//
unsigned long ulSPISlavePin[2] = {SPI_SS_NONE,SPI_SS0};
...
static void xspi_SpiSlavePinSelect_test(void)
{
    ...
    {  
        ...
        xSPISSSet(SPI0_BASE, xSPI_SS_SOFTWARE, ulTemp);
        ulRegVal = xHWREG(SPI0_BASE + SPI_SSR);
    ...
    for(i = 0;i < 2;i++)
    {  
        xSPISSSet(SPI0_BASE,xSPI_SS_HARDWARE, ulSPISlavePin[i]);
        ulRegVal = xHWREG(SPI0_BASE+SPI_SSR);
...
static void xspi_SPIAutoSSEnable_test(void)
{
    ...
    {  
        ulTemp = ulSPISlavePin[i];
        SPIAutoSSEnable(SPI0_BASE, ulSPISlavePin[i]);
        ulRegVal = xHWREG(SPI0_BASE + SPI_SSR);
...
//
    xHWREG(SPI0_BASE + SPI_CNTRL) &= 0x0000;
    for(i = 0;i < 4;i++)
    {
        SPIByteReorderSet(SPI0_BASE,ulByteReorder[i]);
        ulRegVal = xHWREG(SPI0_BASE + SPI_CNTRL) & SPI_CNTRL_REORDER_M;
...
static void xspi_SpiDivOneFunctonSet_test(void)
{
    ...
    //
    SPIDivOneFunction(SPI0_BASE, xtrue);
...
static void xspi_SPIAutoSSDisable_test(void)
{
    ...
    xHWREG(SPI0_BASE + SPI_SSR) &=0x0000;
    SPIAutoSSDisable(SPI0_BASE);
    ulRegVal = xHWREG(SPI0_BASE + SPI_SSR);
...
{
    unsigned long ulTempVal,ulRegVal;
    
    ulTempVal = SPI_SS_EDGE_TRIGGER | SPI_SS_ACTIVE_LOW_FALLING;
    SPISSConfig(SPI0_BASE, SPI_SS_EDGE_TRIGGER, SPI_SS_ACTIVE_LOW_FALLING);
    ulRegVal = xHWREG(SPI0_BASE + SPI_SSR);
    TestAssert((ulTempVal == ulRegVal), "xspi API error!");
    xHWREG(SPI0_BASE + SPI_SSR) &= 0x0000;
    
    ulTempVal = SPI_SS_EDGE_TRIGGER | SPI_SS_ACTIVE_HIGH_RISING;
    SPISSConfig(SPI0_BASE, SPI_SS_EDGE_TRIGGER, SPI_SS_ACTIVE_HIGH_RISING);
    ulRegVal = xHWREG(SPI0_BASE + SPI_SSR);
    TestAssert((ulTempVal == ulRegVal), "xspi API error!");
    xHWREG(SPI0_BASE + SPI_SSR) &= 0x0000;
