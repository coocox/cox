...
static void InputSrcSet(void)
{
    ...
    for (unsigned char i = 0; i < ulLength; i++)
    {
        ...
        TestAssert(Sample_Test[i].ulResult == ulTemp ,"xacmp API error!");
    }
}
...
static void CompHyster( void )
{
    ...
    for(unsigned char i = 0; i < ulLength; i++)
    {
        ...
        TestAssert(Sample_Test[i].ulResult_En == ulTemp, "xacmp API error!");
    }
    for(unsigned char i = 0; i<ulLength; i++)
    {
        ...
        TestAssert(Sample_Test[i].ulResult_Dis == ulTemp, "xacmp API error!"); 
    }	
}
...
unsigned long TestCallback(void *pvCBData, unsigned long ulEvent,
...
//
//*****************************************************************************
static void AcmpCallback(void)
{
    xACMPConfigure(xACMP0_BASE, 0, xACMP_ASRCN_REF);
    
    xACMPIntCallbackInit(xACMP0_BASE, 0, TestCallback);
    xACMPIntEnable(xACMP0_BASE, 0);	
    xIntEnable(xINT_ACMP0);

    xACMPEnable(xACMP0_BASE, 0);

    TestAssertQBreak("T", "xacmp API error!", 0xFFFFFFFF);
}
