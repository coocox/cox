...
//!
//! \param ulBase is the base address of the ACMP.
...
//!
//! When there is any ACMP interrupt occrus, Interrupt Handler will 
...
//! Refrence \ref Mini51_ACMP_Analog_Src_negative.
//!
...
//*****************************************************************************
//
//! \brief Comparator Reference Voltage set.
//!
//! \param ulBase is the base address of the comparator module.
//! \param ucCRVSelect is the CRV module output select that can be selected from
//! these value: \b ACMP_CMPRV_OUTSEL_CRVS, \b ACMP_CMPRV_OUTSEL_1_35V.
//! \param ulCrvs is the Comparator Reference Voltage Setting value(0 - 16).
//!
//! The comparator interrupt is cleared, so that it no longer asserts.  This
//! fucntion must be called in the interrupt handler to keep the handler from
//! being called again immediately upon exit. 
...
//*****************************************************************************
void
ACMPRVCRSet(unsigned long ulBase, unsigned char ucCRVSelect, unsigned long ulCrvs)
{
    xASSERT(ulBase == ACMP_BASE);
    xASSERT((ucCRVSelect == ACMP_CMPRV_OUTSEL_CRVS) || 
            (ucCRVSelect == ACMP_CMPRV_OUTSEL_1_35V));
    
    xASSERT( (ulCrvs >=0) && (ulCrvs <= 16));
    
    xHWREG(ulBase + ACMP_RVCR) &= ~(ACMP_RVCR_OUTSEL_CRVS | ACMP_RVCR_CRVS_M);
    
    if(ucCRVSelect == ACMP_CMPRV_OUTSEL_CRVS)
    {
        //
        // Select the CRVS setting voltage
        //
        xHWREG(ulBase + ACMP_RVCR) |= ACMP_RVCR_OUTSEL_CRVS;
        
        //
        // Comparator Reference Voltage Setting
        //
        xHWREG(ulBase + ACMP_RVCR) |= ulCrvs;        
    }
    else
    {
        //
