...
//
unsigned long ulSYSCTL_CLKSET[] = {1,2,3,4,6,12,14,17,20,25,27,29,31,36,37,40,46,48,50};
...
static void xsysctl_SetHCLK_test(void)
{
    ...
    for(i=0;i<16;i++)
    {
        ...
        ulTemp = xSysCtlClockGet();
        TestAssert(ulTemp == 84000000/(i+1), "xsysctl API error!");
    }
    ...
    //
    xSysCtlClockSet(10000, xSYSCTL_OSC_INTSL|xSYSCTL_OSC_INTSL);
    ulTemp = xSysCtlClockGet();
    ...
    //
    xSysCtlClockSet(22000000, xSYSCTL_OSC_INT|xSYSCTL_OSC_INT);
    ulTemp = xSysCtlClockGet();
    ...
    //
    xSysCtlClockSet(32768, xSYSCTL_OSC_EXTSL|xSYSCTL_OSC_EXTSL);
    ulTemp = xSysCtlClockGet();
