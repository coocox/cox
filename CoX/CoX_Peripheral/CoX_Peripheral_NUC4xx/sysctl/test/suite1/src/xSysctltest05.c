...
static void xsysctl_xSysCtlPeripheralEnable2_test(void)
{
    ...
    ulRegVal = xHWREG(SYSCLK_APBCLK);
    TestAssert((SYSCLK_APBCLK_PWM01_EN == (ulRegVal & SYSCLK_APBCLK_PWM01_EN)), "xsysctl API error!");
	
    xSysCtlPeripheralEnable2(PWMB_BASE);
    ulRegVal = xHWREG(SYSCLK_APBCLK);
    TestAssert((SYSCLK_APBCLK_PWM23_EN == (ulRegVal & SYSCLK_APBCLK_PWM23_EN)), "xsysctl API error!");
#endif
}
