    ...
    for(w=0; w<w1; w++)
    {
        ...
        {   
            ...
            for(s=0; s<s1; s++)
            {
                ...
                xUARTConfigSet(UART_BASE, 115200, ulTemp1);
                xUARTConfigGet(UART_BASE, &ulBuad, &ulConfig);
...
unsigned long uart0CallbackFunc(void *pvCBData, 
                                       ...
                                       void *pvMsgData)
{
    if((xUARTIntStatus(UART0_BASE, xtrue) & 0x100) == 0x100)
    {
        while(!xUARTCharsAvail(UART0_BASE))
            recData[recNum++] = xUARTCharGet(UART0_BASE);
    }  
...
void xuart001Execute_uartInt(void)
{
    ...
    xUARTIntEnable(UART0_BASE, UART_INT_RDA);
...
void xuart002Execute_FIFOConfig(void)
{
    ...
    {
    	xUARTFIFORxLevelSet(UART_BASE, ulRxLevel[i]);
        xUARTFIFORxLevelGet(UART_BASE, &ulTemp1);
    ...
    TestAssert(xUARTSpaceAvail(UART_BASE) == xtrue,
                  "xuart API \"xUARTSpaceAvail\" error!");
    ...
    TestAssert(xUARTCharsAvail(UART_BASE) == xtrue,
                  "xuart API \"xUARTCharsAvail\" error!");
...
void xuart003Execute_IrDAConfig(void)
{
    ...
    xUARTIrDAEnable(UART_BASE);
    ulTemp1 = xHWREG(UART_BASE + UART_FUN_SEL) & 0x02;
    TestAssert(ulTemp1 == 0x02,
                   "xuart API \"UARTEnableIrDA\" error!");
#if 0
    ulTemp1 = UART_CONFIG_IrDA_TX_EN;
    xUARTIrDAConfig(UART_BASE, 115200, ulTemp1, xUART_IRDA_MODE_NORMAL);
...
void xuart004Execute_LinConfig(void)
{
    ...
    xUARTLINConfig(UART_BASE, 115200, ulTemp1);
    ulTemp1 = xHWREG(UART_BASE + UART_ALT_CSR) & 0xCA;
    TestAssert(ulTemp1 == 0xCA,
                   "xuart API \"UARTLINConfig\" error!");

    ulTemp1 = (UART_CONFIG_LIN_RX_EN | UART_CONFIG_LIN_TX_EN | 0x0F);
    xUARTLINConfig(UART_BASE, 115200, ulTemp1);
    ulTemp1 = xHWREG(UART_BASE + UART_ALT_CSR) & 0xCF;
    TestAssert(ulTemp1 == 0xCF,
                   "xuart API \"UARTLINConfig\" error!");

    ulTemp1 = (~(UART_CONFIG_LIN_RX_EN | UART_CONFIG_LIN_TX_EN)) | 0x0A;
    xUARTLINConfig(UART_BASE, 115200, ulTemp1);
    ulTemp1 = xHWREG(UART_BASE + UART_ALT_CSR) & 0x0A;
...
void xuart005Execute_flowControlConfig(void)
{
    ...
    {
        xUARTFlowControlSet(UART_BASE, ulFlowCtlCfg[i]);
        ulTemp1 = xUARTFlowControlGet(UART_BASE);
        TestAssert(ulTemp1 == ulFlowCtlCfg[i],
                   "xuart API \"xUARTFlowControlSet\" or "
...
void xuart006Execute_ModemConfig(void)
{
    ...
    {
        xUARTModemControlSet(UART_BASE, ulModemCfg[i]);
        ulTemp1 = xUARTModemControlGet(UART_BASE, 0);
        ulTemp2 = (((((xHWREG(UART_BASE + UART_MCR)>>1) & 0x01)^
        ((xHWREG(UART_BASE + UART_MCR)>>9) & 0x01)))^1)<<13;
        TestAssert(ulTemp1 == ulTemp2,
                   "xuart API \"xUARTModemControlSet\" or "
                   "\"UARTModemControlGet\" error!");   
    }
    xUARTModemControlClear(UART_BASE, UART_OUTPUT_RTS_SET);
    ulTemp1 = ((xHWREG(UART_BASE + UART_MCR)>>1) & 0x01);
    TestAssert(ulTemp1 == ulTemp2,
                   "xuart API \"xUARTModemControlClear\" error!");
    //
