...
void xuart003Execute_IrDAConfig(void)
{
    ...
    UARTEnableIrDA(UART0_BASE, xtrue);
    ulTemp1 = xHWREG(UART0_BASE + USART_IRCR) & USART_IRCR_IRDA_EN;
    TestAssert(ulTemp1 == USART_IRCR_IRDA_EN, "xuart API  error!");

    UARTEnableIrDA(UART0_BASE, xfalse);
    ulTemp1 = xHWREG(UART0_BASE + USART_IRCR) & USART_IRCR_IRDA_EN;
    TestAssert(ulTemp1 == 0, "xuart API  error!");
    
    ulTemp1 = USART_IRCR_IRDA_EN | USART_IRCR_LB_EN;
    UARTIrDAConfig(UART0_BASE, ulTemp1);
    ulTemp2 = xHWREG(UART0_BASE + USART_IRCR);
    TestAssert(ulTemp1 == ulTemp2, "xuart API error!");

    ulTemp1 = ~(USART_IRCR_IRDA_EN | USART_IRCR_LB_EN);
    UARTIrDAConfig(UART0_BASE, ulTemp1);
    ulTemp2 = xHWREG(UART0_BASE + USART_IRCR);
    ulTemp1 = ulTemp1 & 0x0000FF0F | USART_IRCR_IRDA_EN | USART_IRCR_LB_EN;
