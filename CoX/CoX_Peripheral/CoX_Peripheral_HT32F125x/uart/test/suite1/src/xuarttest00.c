...
void xuart001Execute_NormalConfig(void)
{
    ...
    for(w=0; w<w1; w++)
    {
        for(p=0; p<p1; p++)
        {   
            UARTParityModeSet(UART0_BASE, parity[p]);
            ulTemp1 = UARTParityModeGet(UART0_BASE);
            TestAssert(parity[p] == ulTemp1, "xuart API error!");

            for(s=0; s<s1; s++)
            {
                ulTemp1 = (wordLength[w] | parity[p] | stopbit[s]);
                UARTConfigSetExpClk(UART0_BASE, 115200, ulTemp1);     
    ...
    // Break Control test
    //
    UARTBreakCtl(UART0_BASE, xtrue);
    ulTemp1 = xHWREG(UART0_BASE + USART_LCR) & USART_LCR_BCB;
    TestAssert(ulTemp1 == USART_LCR_BCB, "xuart API \"UARTBreakCtl\" error!");
    
    UARTBreakCtl(UART0_BASE, xfalse);
    ulTemp1 = xHWREG(UART0_BASE + USART_LCR_BCB) & USART_LCR_BCB;
...
void xuart001Execute_FIFOConfig(void)
{
    ...
    for(i=0; i<fifoLevelNum; i++)
    {
        ...
        ulTemp1 = UARTFIFOTriggerLevelGet(UART0_BASE);
    ...
    //
    xHWREG(UART0_BASE + USART_FCR) = USART_FCR_RX_RST | USART_FCR_TX_RST;

    SysCtlDelay(100000);
	xtTemp = UARTFIFOTxIsEmpty(UART0_BASE);

    TestAssert(xtTemp == xtrue, "xuart API error!");

	xtTemp = UARTFIFORxIsEmpty(UART0_BASE);
