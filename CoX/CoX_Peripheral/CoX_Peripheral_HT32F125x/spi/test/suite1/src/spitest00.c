...
// SPI Group Array
//
unsigned long ulSPI[4] = {SPI0_BASE};
...
static void xspi_SpiDataWidthSet_test(void)
{
  ...
  /*  
    ...
    for(i = 0; i < 16; i++)
    {
        ...
        ulRegVal = SPIBitLengthGet(ulSPI[0]);
...
static void xspi_SpiProtocolSet_test(void)
{
    ...
    for(i = 0;i < 4; i++)
    {
        ...
        SPIConfig(ulSPI[0], 18, ulTemp);
        ulRegVal = xHWREG(ulSPI[0] + SPI_CNTRL1) & SPI_CNTRL1_FORMAT_M;
...
static void xspi_SpiSlaveSelect_test(void)
{
    ...
    for(i = 0; i < 1; i++)
    {  
        xSPISSSet(ulSPI[0], xSPI_SS_SOFTWARE, 0);
        ulRegVal = xHWREG(ulSPI[0] + SPI_CNTRL1) & SPI_CNTRL1_SELM;
    ...
    {  
	   ...
	   // xHWREG(ulSPI[0] + SPI_CNTRL1) |= xSPI_SS_HARDWARE |	SPI_MODE_MASTER;
	    xSPISSSet(ulSPI[0], xSPI_SS_HARDWARE, 0);
        ulRegVal = xHWREG(ulSPI[0] + SPI_CNTRL1) & SPI_CNTRL1_SELM;
...
static void xspi_SPISSConfig_test(void)
{
    ...
    for(i = 0; i < 2; i++)
    {
        SPISSConfig(ulSPI[0], ulSelectLevel[i]);
        ulRegVal = xHWREG(ulSPI[0] + SPI_CNTRL1) & SPI_CNTRL1_SELAP;
