...
unsigned long ulxUART0ClkSrc[] = {SYSCTL_PERIPH_UART_S_HCLK, SYSCTL_PERIPH_UART_S_HCLK_2};


unsigned long ulxADC0ClkDiv[] = {1, 2, 4, 8, 16, 32, 64, 6};
...
{
 static char * const pcName = "xsysctl, 002, SysCtl Peripheral Clock Source Set test";
 return pcName;
}
...
static void xsysctl_xSysCtlPeripheralClockSourceSet_test(void)
{
    ...
    for(i = 1; i < 3; i++)
    {
        xSysCtlPeripheralClockSourceSet(xSYSCTL_UART0_HCLK, i);
        ulTemp = xHWREG(SYSCLK_GCFGR);
        TestAssert(((i-1) == ((ulTemp & SYSCLK_GCFGR_URPRE_M)>>SYSCLK_GCFGR_URPRE_S)), 
		                                                    "xsysctl API error!");
    }
    ...
    for(i = 0; i < 8; i++)
    {
        xSysCtlPeripheralClockSourceSet(xSYSCTL_ADC0_HCLK, ulxADC0ClkDiv[i]);
    ...
    for(i = 0; i < 1; i++)
    {
        SysCtlPeripheralClockSourceSet(ulUART0ClkSrc[i]);
        ulTemp = xHWREG(SYSCLK_GCFGR);
    ...
    for(i = 0; i < 8; i++)
    {
        SysCtlPeripheralClockSourceSet(ulADC0ClkSrc[i]);
        ulTemp = xHWREG(SYSCLK_APBCFGR);
        TestAssert((i == ((ulTemp & SYSCLK_APBCFGR_ADCDIV_M)>>SYSCLK_APBCFGR_ADCDIV_S)),
		                                                          "xsysctl API error!");
    }
