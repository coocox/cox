...
{
    SysCtlPeripheralEnable(xSYSCTL_PERIPH_UART1);
}
...
{	  
    ...
    for(i=0; i<4; i++)
	{
	    UARTModeSet(UART1_BASE, ulUartMode[i]);
        ulTemp = xHWREG(UART1_BASE + USART_MDR) & USART_FUN_SEL_M;
...
/*
    ...
    UARTSyncModeConfig(UART1_BASE, 115200, (UART_CONFIG_WLEN_8 | 
                                            UART_CONFIG_STOP_ONE | 
                                            UART_CONFIG_PAR_NONE), ulTemp1);
    ulTemp2 = xHWREG(UART1_BASE + USART_SYNCR);
    TestAssert(ulTemp1 == ulTemp2, "xuart API error!");
...
{	  
    ...
    UART485Config(UART1_BASE, 115200, ulWriteVal, (UART_CONFIG_WLEN_8 | 
                                                   UART_CONFIG_STOP_ONE | 
                                                   UART_CONFIG_PAR_NONE));
	ulReadVal = xHWREG(UART1_BASE + USART_RS485CR);
	...
	ulWriteVal =  USART_RS485CR_NMM_EN;
    UART485Config(UART1_BASE, 115200, ulWriteVal, (UART_CONFIG_WLEN_8 | 
                                                   UART_CONFIG_STOP_ONE | 
                                                   UART_CONFIG_PAR_NONE));
	ulReadVal = xHWREG(UART1_BASE + USART_RS485CR);
	...
	ulWriteVal =  USART_RS485CR_AAD_EN;
    UART485Config(UART1_BASE, 115200, ulWriteVal, (UART_CONFIG_WLEN_8 | 
                                                   UART_CONFIG_STOP_ONE | 
                                                   UART_CONFIG_PAR_NONE));
	ulReadVal = xHWREG(UART1_BASE + USART_RS485CR);
