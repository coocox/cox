...
//! - \p Board: HT32F1755/2755 Development board <br><br>
...
// PDMA Requests
//
static unsigned long ulPDMARequests[6] = {TIMER_PDMA_CH0CCDE, 
                                          TIMER_PDMA_CH1CCDE,
										  TIMER_PDMA_CH2CCDE,
										  TIMER_PDMA_CH3CCDE,
										  TIMER_PDMA_UEVDE,
...
//
//*****************************************************************************
const char* xTimer001GetTest(void)
{
    static const char * const pcName = "xTimer [001]: xtimer register read and write test";
    return pcName;
}
...
static void xTimer001Execute(void)
{
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        for(j = 0; j < 4; j++)
        {
            ...
            ulTemp = xHWREG(ulBase + TIMER_ICTR) & (TIMER_DICTR_CH0CCIE << ulTimerChannel[j]);
            TestAssert(ulTemp == TIMER_DICTR_CH0CCIE << ulTimerChannel[j], 
		...
		*/
		...
		for(k = 0; k < 6; k++)
		{
		    TimerPDMARequestEnable(ulBase, ulPDMARequests[k]);
			ulTemp = xHWREG(ulBase + TIMER_MDCFR) & ulPDMARequests[k];
            TestAssert(ulTemp == ulPDMARequests[k], 
                       "xtimer API \" PDMA Request enable \" error");
		}

		//
		// PDMA Request disable
		//
		for(k = 0; k < 6; k++)
		{
		    TimerPDMARequestDisable(ulBase, ulPDMARequests[k]);
			ulTemp = xHWREG(ulBase + TIMER_MDCFR) & ulPDMARequests[k];
            TestAssert(ulTemp == 0, 
                       "xtimer API \" PDMA Request disable \" error");
		}

		//
		// PDMA Request event select
		//
		TimerPDMARequestEventSet(ulBase, TIMER_CHCCDS_UEV);
		ulTemp = xHWREG(ulBase + TIMER_CTR) & TIMER_CTR_CHCCDS;
        TestAssert(ulTemp == TIMER_CTR_CHCCDS, 
                   "xtimer API \" PDMA Request event select \" error");
    }       
