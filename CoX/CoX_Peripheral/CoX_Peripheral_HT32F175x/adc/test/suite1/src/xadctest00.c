...
static void adcTriggerEnTest(void)
{
	...
	// BFTM trigger
	//
    ADCBftmEventTrigger(ADC_BASE);
	ulReadVal = (xHWREG(ADC_BASE + ADC_TCR) & ADC_TCR_BFTM_EN);
	TestAssert(ulReadVal == ADC_TCR_BFTM_EN, "xadc API error!" );

}

//*****************************************************************************
//
//! \brief  adc api configure test of DMA Enable.
//!
//! \return None.
//
//*****************************************************************************
static void adcDMAEnTest(void)
{
    unsigned long ulDMAMode[3] = {ADC_DMA_SINGAL, ADC_DMA_SUBGROUP, ADC_DMA_CYCLE};
	unsigned long i, ulReadVal;
	...
	for(i=0; i<3; i++)
	{
        ADCDMAEnable(ADC_BASE, ulDMAMode[i]);
        ulReadVal = xHWREG(ADC_BASE + ADC_DMAR);
	    TestAssert(ulDMAMode[i] == (ulDMAMode[i] & ulReadVal), "xadc API error!" );
	}

    //
	// Test ADC DMA Disable
	//
	for(i=0; i<3; i++)
	{
        ADCDMADisable(ADC_BASE, ulDMAMode[i]);
        ulReadVal = xHWREG(ADC_BASE + ADC_DMAR);
	    TestAssert(0 == (ulDMAMode[i] & ulReadVal), "xadc API error!" );
	}
...
//*****************************************************************************
//
//! \brief  adc api test of ADC Input Channel n Offset Cancellation Enable.
//!
//! \return None.
//
//*****************************************************************************
static void ADCInputOffsetCancEnableTest(void)
{
	unsigned long i, ulReadVal;

	//
	// Test ADC Input Channel n Offset Cancellation Enable 
	//
	for(i=0; i<8; i++)
	{
        ADCInputOffsetCancEnable(ADC_BASE, i);
        ulReadVal = xHWREG(ADC_BASE + ADC_OFR0 + i*4);
	    TestAssert(ADC_ADOF0_EN == (ADC_ADOF0_EN & ulReadVal), "xadc API error!" );
	}

    //
	// Test ADC Input Channel n Offset Cancellation Disable
	//
	for(i=0; i<8; i++)
	{
        ADCInputOffsetCancDisable(ADC_BASE, i);
        ulReadVal = xHWREG(ADC_BASE + ADC_OFR0 + i*4);
	    TestAssert(0 == (ADC_ADOF0_EN & ulReadVal), "xadc API error!" );
	}
